  ...
  if (!mounted || !isHydrated) {
    return (
      <div className="h-screen bg-[#010206] flex items-center justify-center">
        <ProbixLogo size="md" className="animate-pulse" />
      </div>
    );
  }
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-[#010206] text-white selection:bg-primary/40">
      ...
      <AnimatePresence mode="wait">
        {view === 'landing' && (
          ...
          >
            {/* HERO SECTION WITH PROBIX AFRICA MAP */}
            <section className="relative w-full max-w-7xl px-12 py-20 flex flex-col md:flex-row items-center min-h-[600px]">
                <div className="relative z-10 flex-[1.2] text-left space-y-10">
                    <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="flex items-center gap-3">
                        <ProbixLogo size="sm" />
                        <span className="text-xs font-black uppercase tracking-[0.4em] opacity-40">PROBIX Terminal Active</span>
                    </motion.div>
                    ...
                    <p className="text-2xl text-slate-400 font-medium max-w-xl leading-relaxed italic">
                        Join thousands of Africans forecasting the future with PROBIX data, insights and collective intelligence.
                    </p>
                ...
                {/* THE PROBIX AFRICA MAP COMPONENT */}
            ...
            <section className="w-full max-w-7xl px-12 py-20 flex flex-col xl:flex-row gap-12">
                <div className="flex-[3] space-y-16">
                    ...
                    <div className="space-y-10">
                        <div className="flex justify-between items-center px-2">
                            ...
                            <button className="text-xs font-bold text-primary uppercase tracking-widest hover:underline">View all hubs</button>
                        </div>
                    ...
                    <div className="space-y-10">
                        <div className="flex justify-between items-center px-2">
                            ...
                            <button className="text-xs font-bold text-primary uppercase tracking-widest hover:underline">View Node Registry</button>
                        </div>
                        ...
                        <div className="flex justify-center gap-4 pt-6">
                            <button className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center hover:bg-white/5 transition-all active-press"><ChevronLeft size={24}/></button>
                            <button className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center hover:bg-white/5 transition-all active-press"><ChevronRight size={24}/></button>
                        </div>
                    </div>
                </div>

                {/* THE PROBIX ANALYTICAL SIDEBAR */}
                <aside className="flex-1 space-y-10">
                    ...
                    <div className="glass p-10 rounded-[40px] border-white/5 space-y-10 relative overflow-hidden bg-white/[0.01]">
                        <div className="flex justify-between items-center px-2">
                             <h4 className="text-sm font-black italic uppercase tracking-widest opacity-60">PROBIX Confidence</h4>
                             <button className="text-[10px] font-black text-primary uppercase tracking-widest">Live Feed</button>
                        </div>
                        <div className="relative w-full aspect-square flex flex-col items-center justify-center">
                            ...
                            <div className="absolute flex flex-col items-center space-y-1 text-center">
                                ...
                                <span className="text-xs font-black uppercase tracking-[0.4em] opacity-40 italic">Overall Market Weight</span>
                                <div className="flex items-center gap-2 text-secondary text-xs font-black italic pt-4">
                                    <TrendingUp size={14}/> + 4.3% drift
                                </div>
                            </div>
                        </div>
                        ...
                        <button className="w-full text-[10px] font-black uppercase tracking-[0.3em] text-primary hover:underline py-2">View Node Audit →</button>
                    </div>

                    {/* TOP PROBIX ANALYSTS */}
                    <div className="glass p-10 rounded-[40px] border-white/5 space-y-10 bg-white/[0.01]">
                        <div className="flex justify-between items-center px-2">
                             <h4 className="text-sm font-black italic uppercase tracking-widest opacity-60">Master Oracles</h4>
                             <button className="text-[10px] font-black text-primary uppercase tracking-widest">Registry</button>
                        </div>
                    ...
                    <div className="relative p-10 rounded-[40px] bg-[#020617] border border-primary/20 overflow-hidden shadow-2xl group active-press">
                        ...
                        <div className="relative z-10 space-y-6 text-left">
                            <h4 className="text-3xl font-black italic uppercase tracking-tight leading-none text-white">Join the protocol.</h4>
                            <p className="text-sm font-medium text-slate-400 italic leading-relaxed">Establish your first forecast and earn oracle reputation.</p>
                            <Button className="w-full !rounded-2xl !py-6 text-xs font-black uppercase tracking-widest italic group active-press">
                                Initialize Forecast <ArrowRight className="ml-3 group-hover:translate-x-2 transition-transform" />
                            </Button>
                        </div>
                ...
                <div className="max-w-7xl mx-auto px-12 space-y-12">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-12 opacity-50 grayscale hover:grayscale-0 transition-all duration-1000">
                        {['Flutterwave', 'Paystack', 'Big Cabal Media', 'Chipper', 'Cowrywise', 'Andela'].map(name => (
                            <span key={name} className="text-3xl font-black italic tracking-tighter uppercase opacity-60 hover:opacity-100 transition-opacity cursor-pointer text-white">{name}</span>
                        ))}
                    </div>
                </div>
            </section>
          ...
          <motion.section key="auth" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="min-h-screen flex items-center justify-center p-6 pt-24 bg-[#010206]">
            <div className="w-full max-w-md p-12 glass rounded-[48px] border-white/10 shadow-3xl text-left relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center text-center mb-10">
                 <ProbixLogo size="md" />
                 <h2 className="text-4xl font-black italic mt-6 uppercase tracking-tighter text-white">{authMode === 'login' ? 'Authorize' : 'Initialize'}</h2>
              ...
              <div className="space-y-6 relative z-10">
                ...
                <Button className="w-full !py-8 text-xl !rounded-[32px] font-black italic uppercase tracking-widest shadow-glow mt-4 active-press" onClick={() => { login(); router.push('/dashboard'); }}>
                  {authMode === 'login' ? 'Seal Session' : 'Access Node'}
                </Button>
                <button onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} className="w-full text-center text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-primary transition-colors italic active-press">
        ...
        <div className="glass p-8 rounded-[40px] border-white/5 flex items-start gap-8 bg-white/[0.01] hover:bg-white/[0.03] transition-all group cursor-default shadow-xl">
            ...
            <div className="text-left space-y-2 flex-1">
                <div className="flex flex-col">
                    <span className="text-3xl font-black italic tracking-tighter tabular leading-none text-white">{title}</span>
                    <span className="text-[10px] font-black uppercase tracking-widest opacity-40 mt-1 italic text-slate-400">{label}</span>
                </div>
                <div className={`text-[9px] font-black italic uppercase tracking-widest ${trend.includes('+') ? 'text-secondary' : 'text-crimson'} opacity-80 pt-1 tabular`}>{trend}</div>
            </div>
        </div>
    );
}
    ...
    return (
        <div className="space-y-3 px-1">
            <div className="flex justify-between items-end">
                <span className="text-[10px] font-black uppercase tracking-widest opacity-60 italic text-slate-400">{label}</span>
                <span className="text-lg font-black italic tabular text-white">{value}%</span>
            </div>
...
function AnalystRowMini({ rank, name, accuracy, followers, img, color }: any) {
    return (
        <div className="flex items-center justify-between group cursor-pointer active-press">
            ...
            <button className="bg-primary/10 text-primary border border-primary/20 px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-primary hover:text-white transition-all shadow-glow active-press">Follow</button>
        </div>
    );
}
