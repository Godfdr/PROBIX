            ...
            <div className="w-full max-w-7xl px-12 z-10 flex flex-col items-start text-left relative">
                <div className="max-w-2xl">
                    <motion.h1
                      ...
                      className="text-7xl md:text-8xl font-black tracking-tighter leading-[0.9] mb-10 italic text-probix-text dark:text-white uppercase"
                    >
                    ...
                    >
                      ...
                      <button className="flex items-center gap-3 px-8 py-5 rounded-full border border-probix-border dark:border-white/10 hover:border-probix-border/30 dark:hover:border-white/20 bg-probix-surface dark:bg-white/5 transition-all group/play">
                        <div className="w-8 h-8 rounded-full border border-probix-border dark:border-white/20 flex items-center justify-center group-hover/play:border-primary transition-colors">
                            <CirclePlay size={16} className="text-probix-text dark:text-white group-hover:text-primary transition-colors" />
                        </div>
                        <span className="text-sm font-black uppercase italic tracking-widest text-probix-text dark:text-white group-hover:text-primary transition-colors">How Probix works</span>
                      </button>
                    </motion.div>
                </div>
...
function OnboardingView({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} className="min-h-screen flex items-center justify-center p-12 bg-probix-bg transition-colors duration-1000">
      <div className="max-w-6xl w-full glass rounded-[72px] p-20 flex flex-col lg:flex-row gap-16 items-center shadow-[0_0_120px_rgba(0,0,0,0.1)] dark:shadow-[0_0_120px_rgba(0,0,0,1)] relative overflow-hidden">
        ...
        <div className="flex-1 space-y-10 relative z-10 text-left">
          <motion.div key={step} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className="inline-block p-6 rounded-[32px] bg-primary/10 border border-primary/20 shadow-inner mb-4">{steps[step].icon}</div>
            <h2 className="text-6xl md:text-7xl font-black italic tracking-tighter leading-[0.9] text-probix-text uppercase">{steps[step].title}</h2>
            <p className="text-2xl text-probix-muted font-bold italic leading-relaxed opacity-80 max-w-lg">{steps[step].desc}</p>
          </motion.div>
          <div className="flex gap-8 pt-8">
            <Button size="lg" className="flex-1 !rounded-[32px] !py-8 text-2xl uppercase italic font-black shadow-glow active:scale-95 shadow-primary/30" onClick={() => step < 2 ? setStep(s => s + 1) : onComplete()}>
              {step < 2 ? "Continue" : "Get Started"}
            </Button>
            <Button size="lg" variant="secondary" className="px-12 !rounded-[32px] glass !py-8 text-2xl uppercase italic font-black active:scale-95">Skip</Button>
          </div>
        ...
        <div className="hidden lg:flex flex-1 justify-center relative">
          <div className="w-[500px] h-[500px] relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
               <motion.div animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 4, repeat: Infinity }} className="w-56 h-56 bg-probix-surface rounded-[56px] border border-probix-border dark:border-white/10 shadow-3xl flex items-center justify-center z-10 shadow-primary/20">
                  <ProbixLogo size="lg" />
               </motion.div>
               <div className="absolute w-[400px] h-[400px] bg-primary/20 rounded-full blur-[150px] -z-0" />
            </div>
          </div>
        </div>
      </div>
...
function AuthView({ mode, toggleMode, onComplete }: AuthViewProps) {
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="min-h-screen flex items-center justify-center p-8 bg-probix-bg transition-colors duration-1000">
      <div className="max-w-[480px] w-full glass rounded-[56px] p-16 border-probix-border dark:border-white/10 shadow-[0_0_150px_rgba(0,0,0,0.05)] dark:shadow-[0_0_150px_rgba(0,0,0,1)] relative overflow-hidden">
        ...
        <div className="text-center mb-12 relative z-10 flex flex-col items-center">
          <ProbixLogo size="md" />
          <h2 className="text-5xl font-black italic tracking-tighter mb-4 mt-10 leading-none text-probix-text uppercase">
        ...
        <div className="space-y-6 relative z-10 text-left">
          <div className="space-y-2 px-1">
            ...
            <div className="relative group">
               <User className="absolute left-6 top-1/2 -translate-y-1/2 text-primary opacity-50 group-focus-within:opacity-100 group-focus-within:text-primary transition-all" size={18}/>
               <input
                  type="text"
                  placeholder="Username or Veltra ID"
                  className="w-full bg-probix-surface/50 border border-probix-border rounded-[28px] py-6 pl-16 pr-8 outline-none focus:border-primary/50 focus:bg-probix-surface transition-all font-black italic text-lg placeholder:opacity-20 text-probix-text shadow-inner"
               />
            </div>
          </div>
          <div className="space-y-2 px-1">
            ...
            <div className="relative group">
               <Lock className="absolute left-6 top-1/2 -translate-y-1/2 text-primary opacity-50 group-focus-within:opacity-100 group-focus-within:text-primary transition-all" size={18}/>
               <input
                  type="password"
                  placeholder="••••••••••••••••"
                  className="w-full bg-probix-surface/50 border border-probix-border rounded-[28px] py-6 pl-16 pr-8 outline-none focus:border-primary/50 focus:bg-probix-surface transition-all font-black italic text-lg placeholder:opacity-20 text-probix-text shadow-inner"
          ...
          <Button size="lg" className="w-full !py-7 text-2xl mt-8 !rounded-[32px] italic font-black tracking-[0.3em] shadow-glow active:scale-95 shadow-primary/40 uppercase" onClick={onComplete}>
          ...
          <div className="relative py-8 flex items-center">
            ...
            <span className="px-6 text-[9px] font-black text-probix-muted uppercase tracking-[0.6em] whitespace-nowrap opacity-60">OR SYNC ACCOUNT</span>
            <div className="flex-1 h-px bg-probix-border" />
          </div>

          <div className="grid grid-cols-2 gap-4">
             <Button variant="secondary" className="!rounded-[24px] !py-5 glass italic font-black text-[10px] tracking-[0.15em] uppercase hover:bg-primary/10 transition-all border-probix-border dark:border-white/5">Veltra Wallet</Button>
             <Button variant="secondary" className="!rounded-[24px] !py-5 glass italic font-black text-[10px] tracking-[0.15em] uppercase hover:bg-fuchsia/10 transition-all border-probix-border dark:border-white/5">Google Node</Button>
          </div>
        </div>

        <p className="text-center mt-12 text-sm font-bold text-probix-muted italic">
